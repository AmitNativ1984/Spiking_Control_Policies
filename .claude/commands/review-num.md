---
description: Numerical stability bug review (NaN/Inf, gradients, loss scaling, dtype, device)
argument-hint: "[file or directory — defaults to the open file, else the uncommitted diff]"
---

# /review-num — Numerical Stability Bug Review

Run the **`review-num`** agent. Do not perform the review yourself in this context.
...
Spawn the `review-num` agent with the resolved target passed explicitly as absolute paths. Run it in the background unless the next thing you do depends on the result.
